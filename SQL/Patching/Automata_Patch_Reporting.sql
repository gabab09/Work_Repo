...
case when (automata_execution_rfc.ExecutionId) is NOT NULL 
then 
(select DATE_SUB(ipexec.created, Interval 8 HOUR) from IPautomata.execution ipexec where ipexec.execution_id = automata_execution_rfc.ExecutionId)
else '-' end as "ExecutionRunTime"
from 
