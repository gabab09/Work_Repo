...
FROM
  (SELECT e.execution_id ,
   ...
   FROM IPautomata.execution e, IPautomata.automaton auto, IPautomata.automaton orig
   WHERE 
	
	 e.automaton_id = auto.automaton_id
	 AND auto.original_id = orig.original_id
	 AND orig.original_id = 14018
     AND e.created >= DATE_SUB(NOW(), INTERVAL 30 DAY)
...
WHERE d.name NOT LIKE ('%PSRemoting%')
