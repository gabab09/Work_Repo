SET @start_date = '2016-09-01 00:00:00';

select 
reporting_table.ClientName,
reporting_table.ActualPurpose,
#reporting_table.TicketOrigin,
#reporting_table.OSType,

count(*) as 'Total', 
#alert_volume.AlertVolume,
CONVERT(((count(*)/alert_volume.AlertVolume)*100),DECIMAL(4,0)) as 'Percentage'
...
then ('True')
when (select z.value from IPcmdb.boolean_datum z,IPcmdb.ci_attribute y,IPcmdb.attribute_definition x,IPradar.ipcmdb_ticket_mapping w where z.datum_id = y.datum_id and y.attribute_definition_id = x.attribute_definition_id and y.ci_id = w.ci_id and x.name = "Monitored" and w.ticket_id = a.ticket_id LIMIT 1) = 0
then ('False')
else ('NA')
end as 'Monitored',
case
...
(select count(q.ticket_id) from IPautomata.execution q where q.ticket_id= a.ticket_id and q.status not in ('AUTO_ABORTED') and (q.creator_id is NULL OR q.creator_id = (select LoginID from auth.LOGIN where Username = 'ipcal')) group by q.ticket_id) as 'NumberOfAutoamta',
...
(select GROUP_CONCAT(q.purpose SEPARATOR ', ') from IPautomata.execution q where q.ticket_id= a.ticket_id and q.status not in ('AUTO_ABORTED') and  (q.creator_id is NULL OR q.creator_id = (select LoginID from auth.LOGIN where Username = 'ipcal')) group by q.ticket_id) as 'Purpose',

(select GROUP_CONCAT(q.status SEPARATOR ', ') from IPautomata.execution q where q.ticket_id= a.ticket_id and q.status not in ('AUTO_ABORTED') and  (q.creator_id is NULL OR q.creator_id = (select LoginID from auth.LOGIN where Username = 'ipcal')) group by q.ticket_id) as 'Status',
...
and a.master_ticket_id is NULL) reporting_table , 

(select count(*) as 'AlertVolume'
from
IPradar.tickets_resolved table2
where 
table2.description not like ('IPpm%')
and table2.create_date >= @start_date
and table2.master_ticket_id is NULL ) alert_volume

#where 
#reporting_table.IPsoftManaged = 'True' 
#and 
#alert_volume.IPsoftManaged = 'True'  
